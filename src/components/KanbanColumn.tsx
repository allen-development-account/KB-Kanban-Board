...
import { MoreHorizontal, Clock, Paperclip, MessageSquare, ListTodo, CheckCircle2 } from 'lucide-react';
...
export function KanbanColumn({ data, onCardClick }: { data: any, onCardClick: (card: any) => void }) {
  return (
            ...
            {data.cards.map((card: any, index: number) => (
              <Draggable key={card.id} draggableId={card.id} index={index}>
                {(provided, snapshot) => (
                  ...
                  >
                    <KanbanCard card={card} isDragging={snapshot.isDragging} onClick={() => onCardClick(card)} />
                  </div>
                )}
              </Draggable>
            ))}
...
function KanbanCard({ card, isDragging, onClick }: { card: any, isDragging?: boolean, onClick: () => void }) {
  return (
    <div 
      onClick={onClick}
